...
class RegistrationController extends Controller
{
    ...
    {
        $registrations = Registration::with('wbp')
        ...
        $registrations->map(function($reg) use (&$dateCounts) {
            ...
            // Only attempt to find WBP if it's not already loaded (e.g. no inmate_number, which is rare if we use standard flow)
            if (!$reg->relationLoaded('wbp') || !$reg->wbp) {
                if (!$reg->inmate_number && $reg->inmate_name) {
